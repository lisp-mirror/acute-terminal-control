...
;Copyright 2018,2019 Prince Trippy programmer@verisimilitudes.net .
...
;Instead, this is a purely practical machinery and I can only think about how nice it would've looked if things hadn't been designed by idiots.
                                  (t (let ((first read) (second read)) ;This is a quick hack for immediate xterm function key support.
                                       (and first second (char= second #.(code-char #x7E))
                                            (if (= #x31 (char-code third))
                                                (case (char-code first)
                                                  (#x35 (cons :function 5)) (#x37 (cons :function 6))
                                                  (#x38 (cons :function 7)) (#x39 (cons :function 8)))
                                                (if (= #x32 (char-code third))
                                                    (case (char-code first)
                                                      (#x30 (cons :function 9)) (#x31 (cons :function 10))
                                                      (#x33 (cons :function 11)) (#x34 (cons :function 12)))))))
                                     ))))
